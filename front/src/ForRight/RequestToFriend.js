...
export default function RequestToFriend () {
  return (
...
const fetchNewFriends = async () => {
  const res = await fetch('http://localhost:3000/api/other/pre-friends', { method: 'GET', // eslint-disable-line object-curly-newline
...
function Friend () {
  const { isLoading, error, data } = useQuery('posts', fetchNewFriends)
  ...
  const acceptFriend = async (e) => {
    try {
      ...
      if (result === true) {
        await axios(`http://localhost:3000/api/other/accept-friend/${Number(e.target.name)}`, { method: 'POST', // eslint-disable-line object-curly-newline
          headers: { Authorization: localStorage.getItem('token') } }).then(res => { // eslint-disable-line object-curly-newline
          alert('You accepted friend')
        })
      } else {
    ...
    return (
      <ul className="content">
        {data.slice(0, visible).map(user => <li key={user.friend_id} className="for-li">
          ...
          <button className="sign" name={user.user_id} onClick={acceptFriend}>Accept friend</button>
        </li>)}
